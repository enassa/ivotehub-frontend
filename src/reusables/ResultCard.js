...
class ResultsCard extends Component {
  ...
  return (
    ...
    <div className={`d-flex f-column j-start a-center width-300 height-200 elevated-all  curved-corners nate-white-bg `}>
      <ButtonBase style={{width:"100%", height:"100%", borderRadius:10}}>
            <div className={` elevated-all round-up wrap-around margin-l-20 margin-r-30`} >
                <Avatar alt={contestant.name} src={userProfile} style={{width:70, height:70, }} />
            </div>
    ...
    <div  className={`d-flex f-column j-space-between  a-start full-height margin-l-30`}>
        <span  className={`padding-l-20 elevated-card text-info-holder min-width-200 min-height-50`}>{`Position: ${contestantId}`}</span>
        <span  className={`padding-l-20 elevated-card text-info-holder min-width-200 min-height-50`}>{`Toal Votes: ${contestant.votes}`}</span>
        <span  className={`padding-l-20 elevated-card text-info-holder min-width-200 min-height-50`}>{`Percentage: N/A`}</span>
    </div>
</div>
    )
  }
